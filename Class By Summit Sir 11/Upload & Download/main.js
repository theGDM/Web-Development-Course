(function () {
    ...
    let rootPath = divBreadCrumb.querySelector("#breadcrumb [purpose='path']")
    ...
    rootPath.addEventListener("click", vieFolderFromPath);
    appClose.addEventListener("click", closeApp);

    //closing the content of file, when we click cross
    function closeApp() {
        divAppTitle.innerHTML = "Title Will Come Here..";
        divAppTitle.removeAttribute("rid");
    ...
    function downloadNotepad() {
        let fid = parseInt(divAppTitle.getAttribute("rid"));//now extracting the rid of text-file from app-title div
        let resource = resources.find(r => r.rid == fid);
        let divNotepadMenu = this.parentNode;

        let strForDownload = JSON.stringify(resource);//converting resource object to string
        let encodeData = encodeURIComponent(strForDownload);//encoding text string so that we can download
        let dataStr = "data:text/json;charset=utf-8," + encodeData;
        ...
        aDownload.click();//manually click na karna pada, isiliye program
    }

    function uploadNotepad() {
        let file = window.event.target.files[0];
        let reader = new FileReader();
        reader.addEventListener("load", function () { //now the function will be executed when the file will be read by reader object as textfile
            let data = window.event.target.result;
            let resource = JSON.parse(data);//convert string into object
        ...
        reader.readAsText(file);//read from file and give us text file
    }
